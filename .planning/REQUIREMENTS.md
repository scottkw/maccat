...
- [x] **CLI-06**: The interactive `select_computer` menu and the `--rename` / `--no-commit` / `--archive-days` / `--catalog-dir` flags behave exactly as before (non-regression).
...
- [x] **ZSH-04**: README and docs no longer reference `update-list.sh` or byte-parity; maccat is described as the standalone tool.
